...
from .models import Drip, SendDrip, QuerySetRule
from .drips import DripBase, DripMessage
from .utils import get_user_model, unicode
...
class DripsTestCase(TestCase):
    ...
    def test_custom_drip(self):
        ...
        self.assertEqual(0, SendDrip.objects.count()) # confirm nothing sent before

        # send the drip
        drip.send()
        self.assertEqual(2, SendDrip.objects.count()) # got sent

        for sent in SendDrip.objects.all():
